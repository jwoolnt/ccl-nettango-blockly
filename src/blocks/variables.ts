...
Blockly.Extensions.register("dynamic_variable_dropdown", function(this) {
  ...
  const updateDropdown = () => {
    ...
    if (dropdown) {
      ...
      const currentValue = dropdown.getValue() || ""; // Fallback to an empty string if null
  ...
  const updateScopeDropdown = () => {
    ...
    if (dropdown) {
      (dropdown as Blockly.FieldDropdown).getOptions = (): Blockly.MenuOption[] => {
        ...
        VariableRegistry.getCustomScopes().forEach(scope => {
  ...
  block.setOnChange(function(event) {
    ...
    if (event.type === Blockly.Events.BLOCK_CHANGE) {
      ...
      if ((event as Blockly.Events.BlockChange).element === "field") {
        ...
        if ((event as Blockly.Events.BlockChange).name === "VAR_NAME" && (event as Blockly.Events.BlockChange).newValue === "CREATE_NEW") {
    ...
    // Handle the case where breeds have been added or removed
